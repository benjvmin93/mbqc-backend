...
class TestStatevec(unittest.TestCase):
    def test_remove_one_qubit(self):
        ...
        """
        nQubits = 2
        sv = StateVec(nb_qubits=nQubits, output_nodes=[1])

        for i in range(sv.nb_qubits):
            sv.entangle(i, (i + 1) % sv.nb_qubits)
        m_op = meas_op(s_signal=0, t_signal=0, angle=0, plane='XY', vop=0, measurement=0)
        ...
        np.testing.assert_almost_equal(np.abs(sv.psi.flatten().dot(sv2.flatten().conj())), 1)
    ...
    def test_measurement_into_each_XYZ_basis(self):
        ...
        for plane in ['XY', 'YZ', 'XZ']:
            ...
            sv.remove_qubit(k)
            sv2 = StateVec(n-1, [0])
            np.testing.assert_almost_equal(np.abs(sv.psi.flatten() @ sv2.psi.flatten().conj()), 1)
